...
reload1 (){
  ## Script to reload squad server and update
  cd $homedir
  #Terminate the squad service
  pkill -2 SquadServer
  printf '\nWaiting for squad to shutdown...\nPlease stand by!' && sleep 15 & spinner

  if pgrep -x "SquadServer" > /dev/null
    then
      printf '\nPlease stop the squad server first\nSometimes this fails on first
  attempt, run me once more to be sure.\n'
      #pkill -15 SquadServer
    else
      $svrroot/steamcmd.sh +login anonymous +force_install_dir server1 +app_update 403240 validate +exit
      printf '\nStarting the server please wait...\n'
      cd $svrroot && tmux new-session -d -s squadserver "./startserver1.sh"
      cd $homedir
  fi
  ...
  if pgrep -x "SquadServer" > /dev/null
    then
      printf '\nIt looks like Squad is up!\n'
    else
      printf "%s\n" "${red}There may be an issue. Please contact the System Administrator${end}"
  fi
}
...
stop1 (){
  pkill -2 SquadServer
  printf '\nWaiting for squad to shutdown...\nPlease stand by!' && sleep 15
  if pgrep -x "SquadServer" > /dev/null
    then
      printf "${red}
      Something went wrong...
      Contact an admin, shutdown failed.
      ${end}"
      #pkill -15 SquadServer
    else
